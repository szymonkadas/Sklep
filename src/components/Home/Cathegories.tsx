...
export default function Cathegories(props: homePageComponentsData) {
  const cathegories = props.data.map((cathegory: Cathegory) => {
    ...
    if (data.photo.length > 0) {
      return (
        <div
          className="cathgegories__cathegory-block"
          style={localStyle}
          key={`cathegory${cathegory.id}div`}
        >
          <h3 className="cathegories__cathegory-block__title">{data.title}</h3>
          <p className="cathegories__cathegory-block__description">
            {data.description}
          </p>
          <button className="cathegories__cathegory-block__button">
            See More
          </button>
        </div>
      );
